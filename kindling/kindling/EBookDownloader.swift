		...
		while true {
			...
			if let message = await ircConnection.receiveMessage() {
				...
				if message.contains("\u{01}DCC SEND") {
					...
					if let (filename, ip, port, fileSize) =
						ircConnection.parseDCCSendMessage(message)
					{
						print(
							"DCC SEND detected. Starting file download: \(filename), size: \(fileSize) bytes"
						)

						let humanReadableIP = ircConnection.convertDCCIP(ip)
						let fileTransfer = DCCFileTransfer(
							filename: filename,
							senderIP: humanReadableIP, port: port,
							fileSize: fileSize)

						do {
							// Await file download
							let fileData =
								try await fileTransfer.startTransfer()
							print(
								"File download complete. Size: \(fileData.count) bytes."
							)
